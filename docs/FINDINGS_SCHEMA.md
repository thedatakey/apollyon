...
```json
{
  ...
  "findings": [{
    ...
    "case_refs": ["case-output/APO-....json"],
    "path": "app.py",
    "line": 3,
    "snippet": null
  }]
...
The fixed `scope` wording describes both AST validation and the explicitly
counted lexical fallback; taint raises confidence only for modeled
...
`engine`, `confidence`, and `trace`, and adds parser coverage counters. Phase 3
optionally adds `case_refs` without changing the v2 discriminator. Consumers
